...
void fuzzer::collect_execution_results()
{
    ...
    constructed_trace->feasible = iomodels::iomanager::instance().received_message_type != connection::message_type::results_from_kleeient_infeasible; // hack
    if (constructed_trace->feasible) {
...
void fuzzer::prepare_did_trace()
{
    ...
    if (processed_trace != nullptr)
        ...
        case EXECUTION_TRACE_STATE::FUZZING_BITS:
            ...
            { /// REMOVE THIS
                // TODO: dont use iomanager to pass data around
                std::vector<branching_coverage_info> trace;
                for (auto &entry : processed_trace->branching_records)
                {
                    branching_coverage_info info(entry.coverage_info.branching_id);
                    info.covered_branch = entry.coverage_info.covered_branch;
                    trace.push_back(info);
                }
                iomodels::iomanager::instance().set_trace(trace);
                // end of ugly block
            } /// REMOVE THIS
            break;
        default:
    ...
    case EXECUTION_TRACE_STATE::FUZZING_BITS:
        {
            ...
            if (fuzzed_branch.fuzzer->done() || !constructed_trace->feasible)
            {
