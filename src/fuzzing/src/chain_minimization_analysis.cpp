...
natural_8_bit  chain_minimization_analysis::max_partial_variable_corrections() const
{
    return 8U;
}
...
void  chain_minimization_analysis::process_execution_results(execution_trace_pointer const  trace_ptr)
{
    TMPROF_BLOCK();
...
void  chain_minimization_analysis::process_execution_results(branching_function_value_type const  function_value)
{
    switch (progress_stage)
    {
        case SEED:
...
void  chain_minimization_analysis::generate_next_seed()
{
...
void  chain_minimization_analysis::generate_next_partial()
{
...
bool  chain_minimization_analysis::try_to_correct_active_partial_variable()
{
...
void  chain_minimization_analysis::compute_gradient()
{
    INVARIANT(partial_variable_values.size() == types_of_variables.size());
...
void  chain_minimization_analysis::compute_step_variables()
{
...
natural_8_bit  chain_minimization_analysis::compute_current_variable_and_function_value_from_step()
{
    INVARIANT(step_function_values.size() == step_variable_values.size());
...
bool  chain_minimization_analysis::apply_fast_execution_using_cache()
{
    vecb  bits;
...
void  chain_minimization_analysis::collect_bits_of_executed_variable_values(std::function<void(natural_32_bit, bool)> const&  bits_collector) const
{
