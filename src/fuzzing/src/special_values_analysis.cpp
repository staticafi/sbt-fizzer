...
bool  special_values_analysis::is_disabled() const
{
    return false;
}


bool  special_values_analysis::is_mutated_type_index_valid() const
{
...
void  special_values_analysis::start(branching_node* const  node_ptr, natural_32_bit const  execution_id_)
{
    ASSUMPTION(is_ready());
...
void  special_values_analysis::stop()
{
    if (!is_busy())
        return;
...
bool  special_values_analysis::generate_next_input(vecb&  bits_ref)
{
    TMPROF_BLOCK();
    ...
    if (!generate_next_typed_value(bits_ref))
    {
        ...
        {
            if (!n->special_values_performed)
                changed_nodes.insert(n);
            n->special_values_performed = true;
            n->special_values_execution = execution_id;
        }
...
bool  special_values_analysis::write_bits(vecb&  bits_ref, T const  (&values)[N])
{
...
bool  special_values_analysis::generate_next_typed_value(vecb&  bits_ref)
{
...
void  special_values_analysis::process_execution_results(execution_trace_pointer const  trace_ptr, branching_node* const  entry_branching_ptr)
{
    TMPROF_BLOCK();
