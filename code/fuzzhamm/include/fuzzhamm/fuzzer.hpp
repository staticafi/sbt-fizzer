#ifndef FUZZHAMM_FUZZER_HPP_INCLUDED
#   define FUZZHAMM_FUZZER_HPP_INCLUDED
...
using  traces_map = std::unordered_map<execution_trace_hash_code, execution_trace_ptr>;
using  directly_input_dependent_branchings_set = std::unordered_set<location_id>;

using  indirectly_input_dependent_branchings_map = std::unordered_map<location_id, iid_fuzzer_base_ptr>;
...
private:
    ...
    execution_trace_ptr  processed_trace;
    execution_trace_ptr  constructed_trace;
    std::unordered_set<execution_trace_hash_code>  seen_trace_hash_codes;
};
