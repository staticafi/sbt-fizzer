...
public:
    ...
    Value* instrumentIcmp(Value* lhs, Value* rhs, CmpInst* cmpInst, IRBuilder<>& builder);
    Value* instrumentFcmp(Value* lhs, Value* rhs, CmpInst* cmpInst, IRBuilder<>& builder);
    ...
    std::tuple<Value*, Value*> instrumentIntIneq(Value* val1, Value* val2, IRBuilder<>& builder);
    std::tuple<Value*, Value*> instrumentFPIneq(Value* val1, Value* val2, IRBuilder<>& builder);
...
void FizzerPass::init(Module& M) {
    ...
    processBranchFunc = M.getOrInsertFunction("__sbt_fizzer_process_branch", Type::getVoidTy(C), Int32Ty, Int8Ty, DoubleTy);
...
CallInst* FizzerPass::instrumentIntEq(Value* val1, Value* val2, IRBuilder<>& builder) {
    Value* diff = builder.CreateSub(val1, val2);
    return builder.CreateBinaryIntrinsic(Intrinsic::abs, diff, ConstantInt::get(Int1Ty, 0));
}


CallInst* FizzerPass::instrumentFPEq(Value* val1, Value* val2, IRBuilder<>& builder) {
    Value* diff = builder.CreateFSub(val1, val2);
    return builder.CreateBinaryIntrinsic(Intrinsic::fabs, diff, ConstantInt::get(Int1Ty, 0));
}


std::tuple<Value*, Value*> FizzerPass::instrumentIntIneq(Value* val1, Value* val2, IRBuilder<>& builder) {
    // returns {val1 - val2, val2 - val1 + 1}
    Value* diff1 = builder.CreateSub(val1, val2);
    Value* diff2 = builder.CreateAdd(builder.CreateSub(val2, val1), ConstantInt::get(val1->getType(), 1));
    return {diff1, diff2};
}


std::tuple<Value*, Value*> FizzerPass::instrumentFPIneq(Value* val1, Value* val2, IRBuilder<>& builder) {
    // returns {val1 - val2, val2 - val1 + 1}
    Value* diff1 = builder.CreateFSub(val1, val2);
    Value* diff2 = builder.CreateFAdd(builder.CreateFSub(val2, val1), ConstantFP::get(val1->getType(), 1));
    return {diff1, diff2};
}

Value* FizzerPass::instrumentIcmp(Value* lhs, Value* rhs, CmpInst* cmpInst, IRBuilder<>& builder) {
    Value* valTrue;
    Value* valFalse;
    ...
    if (lhs->getType()->isPointerTy()) {
        ...
        // if the value was extended we can't overflow meaning we don't need to cast to a higher type 
        if (!(dyn_cast<ZExtInst>(lhs) || dyn_cast<SExtInst>(lhs) || dyn_cast<ZExtInst>(rhs) || dyn_cast<SExtInst>(rhs))) {

            // extend based on the signedness
            if (CmpInst::ICMP_UGT <= opcode && opcode <= CmpInst::ICMP_ULE) {
                lhs = builder.CreateZExt(lhs, lhs->getType()->getExtendedType());
                rhs = builder.CreateZExt(rhs, rhs->getType()->getExtendedType());
            }
            // treat values tested for equality as signed
            else {
                lhs = builder.CreateSExt(lhs, lhs->getType()->getExtendedType());
                rhs = builder.CreateSExt(rhs, rhs->getType()->getExtendedType());
            }
        }
    }
    ...
    Value* distance = builder.CreateUIToFP(
        builder.CreateSelect(cmpInst, valTrue, valFalse), 
        DoubleTy
    );
...
Value* FizzerPass::instrumentFcmp(Value* lhs, Value* rhs, CmpInst* cmpInst, IRBuilder<>& builder) {
    Value* valTrue;
    Value* valFalse;
    ...
    switch (cmpInst->getPredicate()) {
        ...
        case CmpInst::FCMP_UNO:   ///< 1 0 0 0    True if unordered: isnan(X) | isnan(Y)
        ...
        case CmpInst::FCMP_UEQ:   ///< 1 0 0 1    True if unordered or equal
            ...
            break;
        case CmpInst::FCMP_ONE:   ///< 0 1 1 0    True if ordered and operands are unequal
        ...
        case CmpInst::FCMP_UGT:   ///< 1 0 1 0    True if unordered or greater than
            ...
            break;
        case CmpInst::FCMP_OGE:   ///< 0 0 1 1    True if ordered and greater than or equal
        case CmpInst::FCMP_UGE:   ///< 1 0 1 1    True if unordered, greater than, or equal
...
bool FizzerPass::instrumentModule(Module& M) {
    ...
    for (Function& F : M) {
        for (BasicBlock& BB: F) {
            ...
            if (brInst && brInst->isConditional() && dyn_cast<CmpInst>(brInst->getCondition())) {
                changed = true;
                ...
                Value* coveredBranch = brBuilder.CreateZExt(brInst->getCondition(), Int8Ty);
                brBuilder.CreateCall(processBranchFunc, {location, coveredBranch, distance});
            }
        }
    }
    return changed;
...
static void registerFizzerPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) {
    PM.add(new FizzerPass());
}

static RegisterPass<FizzerPass>
    X("legacy-fizzer-pass", "Fizzer Instrumentation pass", false, false);
...
static RegisterStandardPasses
    RegisterFizzerPass0(PassManagerBuilder::EP_EnabledOnOptLevel0, registerFizzerPass);
